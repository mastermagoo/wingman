...
|------|--------|-------|---------|
| `__init__.py` | ⚠️ BROKEN IMPORTS | 51 | File exists, but imports missing modules (`semantic_analyzer`, `code_scanner`, `dependency_analyzer`, `content_quality_validator`) and contains duplicated blocks |
| `semantic_analyzer.py` | ❌ MISSING | N/A | Not present in repo at `wingman/validation/semantic_analyzer.py` |
| `code_scanner.py` | ❌ MISSING | N/A | Not present in repo at `wingman/validation/code_scanner.py` |
| `dependency_analyzer.py` | ❌ MISSING | N/A | Not present in repo at `wingman/validation/dependency_analyzer.py` |
| `content_quality_validator.py` | ❌ MISSING | N/A | Not present in repo at `wingman/validation/content_quality_validator.py` |
| `composite_validator.py` | ❌ MISSING | N/A | Not present in repo at `wingman/validation/composite_validator.py` |
| `tuning_config.py` | ❌ MISSING | N/A | Not present in repo at `wingman/validation/tuning_config.py` |
...
**Verified contents (current workspace)**:
- Only `__pycache__/*.pyc` files exist under `wingman/wingman/validation/` (no `.py` sources).
**Verdict**: Not usable as source; the canonical `wingman/validation/*.py` modules are missing.
